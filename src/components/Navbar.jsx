  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-slate-950/80 backdrop-blur-md border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        {/* LADO IZQUIERDO: ISOTIPO + NOMBRE */}
        <motion.div 
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          className="flex items-center gap-3 group cursor-pointer"
        >
          {/* Isotipo SVG Hexagonal */}
          <div className="relative w-10 h-10">
            <svg viewBox="0 0 100 100" className="w-full h-full transform group-hover:rotate-[15deg] transition-transform duration-500">
              ...
              <text 
                ...
                className="fill-white font-black text-[26px] tracking-tighter"
              >
                OB
              </text>
            </svg>
          ...
          <div className="flex flex-col leading-none">
            ...
            <span className="text-slate-500 font-mono text-[10px] tracking-[0.2em] uppercase">
              Full Stack Dev
            </span>
          </div>
        </motion.div>
        ...
        <div className="hidden md:flex items-center gap-8 text-slate-400 font-medium">
          ...
          <button 
            ...
            className="flex items-center gap-3 bg-slate-900 border border-slate-800 hover:border-cyan-500/50 px-4 py-2 rounded-full transition-all group overflow-hidden"
          >
            ...
            <span className="text-[10px] font-bold tracking-widest text-slate-300 group-hover:text-cyan-400 uppercase">
              {lang === 'en' ? 'ES' : 'EN'}
            </span>
          </button>
        </div>

        {/* LADO DERECHO: BOTÓN CV */}
        <motion.a 
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          href="/CV_Oscar_Blanco.pdf" 
          download 
          className="bg-cyan-500 hover:bg-cyan-400 text-slate-950 px-6 py-2 rounded-full text-xs font-black uppercase tracking-tighter transition-all shadow-lg shadow-cyan-500/20"
        >
          {t.cv}
        </motion.a>
